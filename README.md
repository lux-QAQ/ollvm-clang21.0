...
目前以及完成了大部分的移植，移植后的新项目[ollvm-plugins](https://github.com/ollvm-adaplite/ollvm-plugins)可能不再会合并到本项目中，其后续可能作为插件式的混淆器。
...
1. 在编译复杂代码，使用`-mllvm -enable-allobf`启用所有混淆器时，会消耗`非常多的内存（已请求内存可达100GB+）`，可能会导致电脑崩溃。这个问题可能是由于代码内存泄漏导致的，我已经根据`ASAN`的报告修复了部分内存泄露，但是我发现这个问题仍然没有彻底解决。故在大型项目请勿启用所有混淆器。
2. 虚假控制流参数`-enable-bcfobf`启用时。代码中不能存在（抛出异常好像可以存在）捕获异常的代码，会导致编译器崩溃。这个问题据说是：`ollvm`中`fixstack`函数导致的，如果要修复可能得修改`eh的返回边`还有`invoke的支配关系`。该问题在新版本已经得到了缓解，但是不保证**100%**正常。
