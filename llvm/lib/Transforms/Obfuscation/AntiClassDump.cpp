...
struct AntiClassDump : public ModulePass {
  ...
  Type *getMethodListPtrType(Module *M) const {
    return GetPointerTypeCompat(
        M->getContext(),
  ...
  void handleClass(GlobalVariable *GV, Module *M) {
    ...
    Value *ClassNameGV = CreateGlobalStringPtrCompat(*IRB, *M, ClassName);
    ...
    if (!metaclassCS->getAggregateElement(5)->isNullValue()) {
      ...
      Constant *bitcastExpr = ConstantExpr::getBitCast(
          newMethodStructGV, getMethodListPtrType(M));
    ...
    if (UseInitialize) {
      MethName = CreateGlobalStringPtrCompat(*IRB, *M, "initialize");
    } else {
      MethName = CreateGlobalStringPtrCompat(*IRB, *M, "load");
    }
    ...
    if (triple.isOSDarwin() && triple.isArch64Bit()) {
      MethType = CreateGlobalStringPtrCompat(*IRB, *M, "v16@0:8");
    } else if (triple.isOSDarwin() && triple.isArch32Bit()) {
      MethType = CreateGlobalStringPtrCompat(*IRB, *M, "v8@0:4");
    } else {
    ...
    for (unsigned int i = 0; i < class_ro->getType()->getNumElements(); i++) {
      ...
      if ((!opaquepointers &&
        ...
        for (unsigned int i = 0; i < methodList->getNumOperands(); i++) {
          ...
          Constant *SELName = CreateGlobalStringPtrCompat(
              *IRB, *M,
          ...
          replaceMethodArgs.emplace_back(CreateGlobalStringPtrCompat(
              *IRB, *M,
              cast<ConstantDataSequential>(
                  cast<GlobalVariable>(
                      opaquepointers
