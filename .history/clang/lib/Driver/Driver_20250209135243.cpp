...
getHIPOffloadTargetTriple(const Driver &D, const ArgList &Args)
{
    if (!Args.hasArg(options::OPT_offload_EQ))
        {
            ...
            if (llvm::is_contained(OffloadArchs, "amdgcnspirv") && OffloadArchs.size() == 1)
    ...
    if (!TT)
        return std::nullopt;
    if (TT->getArch() == llvm::Triple::amdgcn && TT->getVendor() == llvm::Triple::AMD && TT->getOS() == llvm::Triple::AMDHSA)
        return TT;
    ...
    for (const Arg *A : Args.filtered(options::OPT_UNKNOWN))
        {
            ...
            if (getOpts().findNearest(ArgString, Nearest, VisibilityMask) > 1)
                {
                    if (!IsCLMode() && getOpts().findExact(ArgString, Nearest, llvm::opt::Visibility(options::CC1Option)))
                        {
            ...
            ContainsError |= Diags.getDiagnosticLevel(DiagID, SourceLocation()) > DiagnosticsEngine::Warning;
        }
                                 ...
                                 Arg **FinalPhaseArg) const
{
    ...
    if (CCCIsCPP() || (PhaseArg = DAL.getLastArg(options::OPT_E)) || (PhaseArg = DAL.getLastArg(options::OPT__SLASH_EP)) || (PhaseArg = DAL.getLastArg(options::OPT_M, options::OPT_MM)) || (PhaseArg = DAL.getLastArg(options::OPT__SLASH_P)) || CCGenDiagnostics)
        {
    ...
    else if ((PhaseArg = DAL.getLastArg(options::OPT__precompile)) || (PhaseArg = DAL.getLastArg(options::OPT_extract_api)) || (PhaseArg = DAL.getLastArg(options::OPT_fmodule_header, options::OPT_fmodule_header_EQ)))
        {
    ...
    for (Arg *A : Args)
        {
            ...
            // option.
            if ((A->getOption().matches(options::OPT_Wl_COMMA) || A->getOption().matches(options::OPT_Xlinker)) && A->containsValue("--no-demangle"))
                {
            ...
            if (A->getOption().matches(options::OPT_Wp_COMMA) && A->getNumValues() > 0 && (A->getValue(0) == StringRef("-MD") || A->getValue(0) == StringRef("-MMD")))
                {
            ...
            if (A->getOption().matches(options::OPT_l))
                {
                    ...
                    if (!HasNostdlib && !HasNodefaultlib && !HasNostdlibxx && Value == "stdc++")
                        {
...
// didn't specify one.
    ...
    if (!Args.hasArg(options::OPT_mlinker_version_EQ) && strlen(HOST_LINK_VERSION) > 0)
        {
                                        ...
                                        StringRef DarwinArchName = "")
{
    ...
    if (A)
        {
            ...
            if (A->getOption().matches(options::OPT_m64) || A->getOption().matches(options::OPT_maix64))
                {
                    AT = Target.get64BitArchVariant().getArch();
                    if (Target.getEnvironment() == llvm::Triple::GNUX32 || Target.getEnvironment() == llvm::Triple::GNUT64)
                        Target.setEnvironment(llvm::Triple::GNU);
                    else if (Target.getEnvironment() == llvm::Triple::MuslX32)
                        Target.setEnvironment(llvm::Triple::Musl);
                }
            else if (A->getOption().matches(options::OPT_mx32) && Target.get64BitArchVariant().getArch() == llvm::Triple::x86_64)
                {
            ...
            else if (A->getOption().matches(options::OPT_m32) || A->getOption().matches(options::OPT_maix32))
                {
                    AT = Target.get32BitArchVariant().getArch();
            ...
            else if (A->getOption().matches(options::OPT_m16) && Target.get32BitArchVariant().getArch() == llvm::Triple::x86)
                {
                    AT = llvm::Triple::x86;
                    ...
                    if (ABIName == "32")
                        {
                            ...
                            if (Target.getEnvironment() == llvm::Triple::GNUABI64 || Target.getEnvironment() == llvm::Triple::GNUABIN32)
                                Target.setEnvironment(llvm::Triple::GNU);
                        }
                    else if (ABIName == "n32")
                        {
                            Target = Target.get64BitArchVariant();
                            if (Target.getEnvironment() == llvm::Triple::GNU || Target.getEnvironment() == llvm::Triple::GNUT64 || Target.getEnvironment() == llvm::Triple::GNUABI64)
                                Target.setEnvironment(llvm::Triple::GNUABIN32);
                            else if (Target.getEnvironment() == llvm::Triple::Musl || Target.getEnvironment() == llvm::Triple::MuslABI64)
                                Target.setEnvironment(llvm::Triple::MuslABIN32);
                        }
                    else if (ABIName == "64")
                        {
                            Target = Target.get64BitArchVariant();
                            if (Target.getEnvironment() == llvm::Triple::GNU || Target.getEnvironment() == llvm::Triple::GNUT64 || Target.getEnvironment() == llvm::Triple::GNUABIN32)
                                Target.setEnvironment(llvm::Triple::GNUABI64);
                            else if (Target.getEnvironment() == llvm::Triple::Musl || Target.getEnvironment() == llvm::Triple::MuslABIN32)
    ...
    if (Target.isRISCV())
        {
            if (Args.hasArg(options::OPT_march_EQ) || Args.hasArg(options::OPT_mcpu_EQ))
                {
                                 ...
                                 SmallVectorImpl<llvm::Triple> &SYCLTriples)
{
    ...
    for (const auto &SYCLTriple : SYCLTriples)
        {
            if (SYCLTriple.getSubArch() == llvm::Triple::NoSubArch && SYCLTriple.isSPIROrSPIRV())
                return false;
        }
                                              ...
                                              InputList &Inputs)
{
    ...
    bool IsHIP =
        ...
        || C.getInputArgs().hasArg(options::OPT_hip_link) || C.getInputArgs().hasArg(options::OPT_hipstdpar);
    ...
    bool IsOpenMPOffloading =
        ((IsCuda || IsHIP) && UseLLVMOffload) || (C.getInputArgs().hasFlag(options::OPT_fopenmp, options::OPT_fopenmp_EQ, options::OPT_fno_openmp, false) && (C.getInputArgs().hasArg(options::OPT_fopenmp_targets_EQ) || C.getInputArgs().hasArg(options::OPT_offload_arch_EQ)));
    if (IsOpenMPOffloading)
        {
            ...
            else if (C.getInputArgs().hasArg(options::OPT_offload_arch_EQ) && ((!IsHIP && !IsCuda) || UseLLVMOffload))
                {
                    ...
                    for (StringRef Arch : Archs)
                        {
                            if (NVPTXTriple && IsNVIDIAOffloadArch(StringToOffloadArch(getProcessorFromTargetID(*NVPTXTriple, Arch))))
                                {
                                    DerivedArchs[NVPTXTriple->getTriple()].insert(Arch);
                                }
                            else if (AMDTriple && IsAMDOffloadArch(StringToOffloadArch(getProcessorFromTargetID(*AMDTriple, Arch))))
                                {
...
bool Driver::loadDefaultConfigFiles(llvm::cl::ExpansionContext &ExpCtx)
{
    ...
    // compatibility.
    if (!ClangNameParts.TargetPrefix.empty() && computeTargetTriple(*this, "/invalid/", *CLOptions).str() == "/invalid/")
        {
            llvm::Triple PrefixTriple{ClangNameParts.TargetPrefix};
            if (PrefixTriple.getArch() == llvm::Triple::UnknownArch || PrefixTriple.isOSUnknown())
                Triple = PrefixTriple;
        }
    ...
    bool TryModeSuffix = !ClangNameParts.ModeSuffix.empty() && ClangNameParts.ModeSuffix != RealMode;
    if (TryModeSuffix)
        {
            if (findTripleConfigFile(ExpCtx, CfgFilePath, Triple,
    ...
    if (ExpCtx.findConfigFile(CfgFileName, CfgFilePath))
    ...
    else if (TryModeSuffix)
        {
            ...
            if (ExpCtx.findConfigFile(CfgFileName, CfgFilePath) && readConfigFile(CfgFilePath, ExpCtx))
                return true;
        }
...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList)
{
    ...
    ModulesModeCXX20 =
        !Args.hasArg(options::OPT_fmodules) && Std && (Std->containsValue("c++20") || Std->containsValue("c++2a") || Std->containsValue("c++23") || Std->containsValue("c++2b") || Std->containsValue("c++26") || Std->containsValue("c++2c") || Std->containsValue("c++latest"));
    ...
    if (!Triple.isWasm())
        {
            ...
            if (Triple.getEnvironmentVersion().empty() && TripleVersionName != "" && TripleVersionName != TripleObjectFormat)
                {
                                    ...
                                    SmallString<128> &CrashDiagDir)
{
    ...
    assert(llvm::Triple(llvm::sys::getProcessTriple()).isOSDarwin() && "Only knows about .crash files on Darwin");
    ...
    for (fs::directory_iterator File(CrashDiagDir, EC), FileEnd;
         File != FileEnd && !EC; File.increment(EC))
        {
            ...
            int CrashPID;
            if (OpenBracket < 0 || CloseBracket < 0 || ParentProcess.slice(OpenBracket + 1, CloseBracket).getAsInteger(10, CrashPID) || CrashPID != PID)
                {
                    continue;
                }
    ...
    if (llvm::Triple(llvm::sys::getProcessTriple()).isOSDarwin())
        {
            ...
            if (getCrashDiagnosticFile(ReproCrashFilename, CrashDiagDir))
                ...
                {  // Suggest a directory for the user to look for .crash files.
...
void Driver::setUpResponseFiles(Compilation &C, Command &Cmd)
{
    ...
    // skip it.
    if (Cmd.getResponseFileSupport().ResponseKind == ResponseFileSupport::RF_None || llvm::sys::commandLineFitsWithinSystemLimits(Cmd.getExecutable(), Cmd.getArguments()))
        return;
...
void Driver::PrintVersion(const Compilation &C, raw_ostream &OS) const
{
    ...
    const ToolChain &TC = C.getDefaultToolChain();
    OS << "\033[38;5;117mIf you find this project helpful, please consider starring  the following projects:\n\033[0m" << '\n';
    OS << "\033[38;5;194mOllvm21: \033[0m\033[4m\033[3mhttps://github.com/GreenDamTan/llvm-project_ollvm\033[0m" << '\n';
    OS << "\033[38;5;194mHikari: \033[0m\033[4m\033[3mhttps://github.com/61bcdefg/Hikari-LLVM15\033[0m" << '\n';
...
bool Driver::HandleImmediateArgs(Compilation &C)
{
    ...
    if (C.getArgs().hasArg(options::OPT_help) || C.getArgs().hasArg(options::OPT__help_hidden))
        {
    ...
    if (C.getArgs().hasArg(options::OPT_v) || C.getArgs().hasArg(options::OPT__HASH_HASH_HASH) || C.getArgs().hasArg(options::OPT_print_supported_cpus) || C.getArgs().hasArg(options::OPT_print_supported_extensions) || C.getArgs().hasArg(options::OPT_print_enabled_extensions))
        {
            PrintVersion(C, llvm::errs());
...
{
    if (isa<CompileJobAction>(A) || isa<BackendJobAction>(A) || isa<AssembleJobAction>(A))
        return true;
    ...
    for (Action *Act : SingleActions)
        {
            ...
            bool enablesDebugInfo = A && !A->getOption().matches(options::OPT_g0) && !A->getOption().matches(options::OPT_gstabs);
            if ((enablesDebugInfo || willEmitRemarks(Args)) && ContainsCompileOrAssembleAction(Actions.back()))
                {
                                    ...
                                    types::ID Ty, bool TypoCorrect) const
{
    ...
    if (Ty == types::TY_CXXSHeader || Ty == types::TY_CXXUHeader || (ModulesModeCXX20 && Ty == types::TY_CXXHeader))
        return true;
            ...
            if (A->getOption().getKind() == Option::InputClass)
                {
                    ...
                    if (InputType == types::TY_Nothing)
                        {
                            ...
                            if (memcmp(Value, "-", 2) == 0)
                            ...
                            else
                                {
                                    ...
                                    if (Args.hasArgNoClaim(options::OPT_fthinlto_index_EQ) && Ty == types::TY_Object)
                                        Ty = types::TY_LLVM_BC;
                                }
                            ...
                            if ((Ty == types::TY_CXXHeader || Ty == types::TY_CHeader) && hasHeaderMode())
                    ...
                    if ((Ty == types::TY_C || Ty == types::TY_CXX) && Args.hasArgNoClaim(options::OPT_hipstdpar))
                        Ty = types::TY_HIP;
...
class OffloadingActionBuilder final
{
    ...
    public:
        DeviceActionBuilder(Compilation &C, DerivedArgList &Args,
            ...
            : C(C), Args(Args), Inputs(Inputs), AssociatedOffloadKind(AssociatedOffloadKind)
        {
        }
    ...
    protected:
        ...
        struct TargetID
        {
            ...
            const char *ID;
            TargetID(OffloadArch Arch)
            {
                ID = OffloadArchToString(Arch);
            }
    ...
    public:
        ...
        ActionBuilderReturnCode addDeviceDependences(Action *HostAction) override
        {
            ...
            if (auto *IA = dyn_cast<InputAction>(HostAction))
                {
                    assert(!GpuArchList.empty() && "We should have at least one GPU architecture.");

                    // If the host input is not CUDA or HIP, we don't need to bother about
                    // this input.
                    if (!(IA->getType() == types::TY_CUDA || IA->getType() == types::TY_HIP || IA->getType() == types::TY_PP_HIP))
                        {
            ...
            if (auto *UA = dyn_cast<OffloadUnbundlingJobAction>(HostAction))
                {
                    ...
                    if (IA->getType() == types::TY_Object && (!llvm::sys::path::has_extension(FileName) || types::lookupTypeForExtension(llvm::sys::path::extension(FileName).drop_front()) != types::TY_Object || llvm::sys::path::extension(FileName) == LibFileExt))
                        return ABRT_Inactive;
        ...
        void appendTopLevelActions(ActionList &AL) override
        {
            ...
            // architecture.
            assert(CudaDeviceActions.size() == GpuArchList.size() && "Expecting one action per GPU architecture.");
            assert(ToolChains.size() == 1 && "Expecting to have a single CUDA toolchain.");
            for (unsigned I = 0, E = GpuArchList.size(); I != E; ++I)
        ...
        bool initialize() override
        {
            ...
            for (Arg *A : C.getArgsForToolChain(&TC, /*BoundArch=*/"",
                                                AssociatedOffloadKind))
                {
                    if (!(A->getOption().matches(options::OPT_offload_arch_EQ) || A->getOption().matches(options::OPT_no_offload_arch_EQ)))
                        continue;
                    A->claim();
    ...
    public:
                             ...
                             PhasesTy &Phases) override
        {
            ...
            assert(CudaDeviceActions.size() == GpuArchList.size() && "Expecting one action per GPU architecture.");
            assert(!CompileHostOnly && "Not expecting CUDA actions in host-only compilation.");
                    ...
                    for (unsigned I = 0, E = GpuArchList.size(); I != E; ++I)
                        {
                            ...
                            // device-only mode.
                            if (!isa<AssembleJobAction>(CudaDeviceActions[I]) || CompileDeviceOnly)
                                continue;
            ...
            assert(CurPhase < phases::Backend &&
    ...
    public:
                             ...
                             PhasesTy &Phases) override
        {
            ...
            assert(((CurPhase == phases::Link && Relocatable) || CudaDeviceActions.size() == GpuArchList.size()) && "Expecting one action per GPU architecture.");
            assert(!CompileHostOnly && "Not expecting HIP actions in host-only compilation.");

            bool ShouldLink = !EmitReloc || !*EmitReloc;

            if (!Relocatable && CurPhase == phases::Backend && !EmitLLVM && !EmitAsm && ShouldLink)
                {
            ...
            if (CompileDeviceOnly && CurPhase == FinalPhase && BundleOutput && *BundleOutput)
                {
                    for (unsigned I = 0, E = GpuArchList.size(); I != E; ++I)
                        {
                            OffloadAction::DeviceDependences DDep;
            ...
            return (CompileDeviceOnly && (CurPhase == FinalPhase || (!ShouldLink && CurPhase == phases::Assemble)))
                       ? ABRT_Ignore_Host
                       : ABRT_Success;
        }
        ...
        void appendLinkDeviceActions(ActionList &AL) override
        {
            ...
            assert(DeviceLinkerInputs.size() == GpuArchList.size() && "Linker inputs and GPU arch list sizes do not match.");
...
public:
    ...
    void recordHostAction(Action *HostAction, const Arg *InputArg)
    {
        ...
        assert(Loc->second == InputArg && "host action mapped to multiple input arguments");
        (void)Loc;
    }
                                     ...
                                     DeviceActionBuilder::PhasesTy &Phases)
    {
        ...
        // nullptr.
        if (IgnoringBuilders && SpecializedBuilders.size() == (InactiveBuilders + IgnoringBuilders))
            return nullptr;
                                          ...
                                          const Arg *InputArg)
    {
        ...
        if (CanUseBundler && isa<InputAction>(HostAction) && InputArg->getOption().getKind() == llvm::opt::Option::InputClass && (!types::isSrcFile(HostAction->getType()) || HostAction->getType() == types::TY_PP_HIP))
            {
        ...
        for (auto *SB : SpecializedBuilders)
            {
                ...
                assert(RetCode != DeviceActionBuilder::ABRT_Ignore_Host && "Host dependence not expected to be ignored.!");
                               ...
                               const Arg *InputArg)
    {
        ...
        if (CanUseBundler && HostAction && HostAction->getType() != types::TY_Nothing && !OffloadAL.empty())
            {
    ...
    if (FinalPhase == phases::Link)
        {
            ...
            if (C.getDefaultToolChain().getTriple().isWindowsMSVCEnvironment() && LTOMode != LTOK_None && !Args.getLastArgValue(options::OPT_fuse_ld_EQ).starts_with_insensitive("lld"))
                          ...
                          const InputList &Inputs, ActionList &Actions) const
{
    ...
    bool UseNewOffloadingDriver =
        C.isOffloadingHostKind(Action::OFK_OpenMP) || C.isOffloadingHostKind(Action::OFK_SYCL) || Args.hasFlag(options::OPT_foffload_via_llvm, options::OPT_fno_offload_via_llvm, false) || Args.hasFlag(options::OPT_offload_new_driver, options::OPT_no_offload_new_driver, C.isOffloadingHostKind(Action::OFK_Cuda));
                    ...
                    if (Phase == phases::Link)
                        {
                            ...
                            if (!(C.getInputArgs().hasArg(options::OPT_hip_link) && (C.getInputArgs().hasArg(options::OPT_emit_llvm))) && !offloadDeviceOnly())
    ...
    if (!LinkerInputs.empty())
        {
            ...
            else if (UseNewOffloadingDriver || Args.hasArg(options::OPT_offload_link))
                {
            ...
            for (auto &I : Inputs)
                {
                    ...
                    if (InputType == types::TY_IFS || InputType == types::TY_PP_Asm || InputType == types::TY_Asm)
                        continue;
                            ...
                            switch (Phase)
                                {
                                    default:
                                        ...
                                        case phases::IfsMerge: {
                                            assert(Phase == PhaseList.back() && "merging must be final compilation step.");
                                            MergerInputs.push_back(Current);
                                            Current = nullptr;
                                            break;
                                        }
    ...
    for (auto Opt : {options::OPT_print_supported_cpus,
                     ...
                     options::OPT_print_enabled_extensions})
        {
            ...
            if (Arg *A = Args.getLastArg(Opt))
                {
                    if (Opt == options::OPT_print_supported_extensions && !C.getDefaultToolChain().getTriple().isRISCV() && !C.getDefaultToolChain().getTriple().isAArch64() && !C.getDefaultToolChain().getTriple().isARM())
                        {
                            C.getDriver().Diag(diag::err_opt_not_valid_on_target)
                                << "--print-supported-extensions";
                            return;
                        }
                    if (Opt == options::OPT_print_enabled_extensions && !C.getDefaultToolChain().getTriple().isRISCV() && !C.getDefaultToolChain().getTriple().isAArch64())
                        {
                            C.getDriver().Diag(diag::err_opt_not_valid_on_target)
                                        ...
                                        bool SuppressError = false)
{
    ...
    if (!SuppressError && Triple.isNVPTX() && (Arch == OffloadArch::UNKNOWN || !IsNVIDIAOffloadArch(Arch)))
        {
            C.getDriver().Diag(clang::diag::err_drv_offload_bad_gpu_arch)
                << "CUDA" << ArchStr;
            return StringRef();
        }
    else if (!SuppressError && Triple.isAMDGPU() && (Arch == OffloadArch::UNKNOWN || !IsAMDOffloadArch(Arch)))
        {
            C.getDriver().Diag(clang::diag::err_drv_offload_bad_gpu_arch)
                << "HIP" << ArchStr;
            return StringRef();
                                       ...
                                       Action *HostAction) const
{
    ...
    // ignore embedding.
    if (offloadHostOnly() || !types::isSrcFile(Input.first) || !(isa<CompileJobAction>(HostAction) || getFinalPhase(Args) == phases::Preprocess))
        return HostAction;
    ...
    for (Action::OffloadKind Kind : OffloadKinds)
        {
            ...
            if ((Kind == Action::OFK_Cuda && !types::isCuda(InputType)) || (Kind == Action::OFK_HIP && !types::isHIP(InputType)))
                continue;
            ...
            if (isa<CompileJobAction>(HostAction) && Kind == Action::OFK_SYCL && HostAction->getType() != types::TY_Nothing && C.getSingleOffloadToolChain<Action::OFK_Host>()->getTriple().isOSDarwin())
                    ...
                    for (Action *&A : DeviceActions)
                        {
                            ...
                            if (isa<CompileJobAction>(A) && isa<CompileJobAction>(HostAction) && Kind == Action::OFK_OpenMP && HostAction->getType() != types::TY_Nothing)
                                {
            ...
            for (Action *&A : DeviceActions)
                {
                    if ((A->getType() != types::TY_Object && A->getType() != types::TY_LTO_BC) || Kind != Action::OFK_HIP || Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false))
                        continue;
            ...
            for (Action *A : DeviceActions)
                {
                    ...
                    for (Action *Input : A->getInputs())
                        if (Kind == Action::OFK_Cuda && A->getType() == types::TY_Object && !Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false))
    ...
    bool ShouldBundleHIP =
        C.isOffloadingHostKind(Action::OFK_HIP) && Args.hasFlag(options::OPT_gpu_bundle_output, options::OPT_no_gpu_bundle_output, true) && !Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false) && !llvm::any_of(OffloadActions, [](Action *A) { return A->getType() != types::TY_Image; });
    ...
    OffloadAction::DeviceDependences DDep;
    if (C.isOffloadingHostKind(Action::OFK_Cuda) && !Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false))
        {
    ...
    else if (C.isOffloadingHostKind(Action::OFK_HIP) && !Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false))
        {
    ...
    switch (Phase)
        {
            ...
            case phases::IfsMerge:
                ...
                case phases::Preprocess: {
                    ...
                    if (Args.hasArg(options::OPT_M, options::OPT_MM) && !Args.hasArg(options::OPT_MD, options::OPT_MMD))
                        {
                    ...
                    else
                        {
                            ...
                            assert(OutputTy != types::TY_INVALID && "Cannot preprocess this input type!");
                        }
                ...
                case phases::Precompile: {
                    ...
                    if (Args.hasArg(options::OPT_modules_reduced_bmi) && !Args.getLastArg(options::OPT__precompile))
                        return Input;

                    types::ID OutputTy = getPrecompiledType(Input->getType());
                    assert(OutputTy != types::TY_INVALID && "Cannot precompile this input type!");
                ...
                case phases::Backend: {
                    if (isUsingLTO() && TargetDeviceOffloadKind == Action::OFK_None)
                        {
                            types::ID Output;
                            if (Args.hasArg(options::OPT_ffat_lto_objects) && !Args.hasArg(options::OPT_emit_llvm))
                    ...
                    if (Args.hasArg(options::OPT_emit_llvm) || TargetDeviceOffloadKind == Action::OFK_SYCL || (((Input->getOffloadingToolChain() && Input->getOffloadingToolChain()->getTriple().isAMDGPU()) || TargetDeviceOffloadKind == Action::OFK_HIP) && (Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false) || TargetDeviceOffloadKind == Action::OFK_OpenMP)))
                        {
                            types::ID Output =
                                Args.hasArg(options::OPT_S) && (TargetDeviceOffloadKind == Action::OFK_None || offloadDeviceOnly() || (TargetDeviceOffloadKind == Action::OFK_HIP && !Args.hasFlag(options::OPT_offload_new_driver, options::OPT_no_offload_new_driver, C.isOffloadingHostKind(Action::OFK_Cuda))))
                                    ? types::TY_LLVM_IR
                                    : types::TY_LLVM_BC;
                            return C.MakeAction<BackendJobAction>(Input, Output);
                        }
...
void Driver::BuildJobs(Compilation &C) const
{
    ...
    if (FinalOutput)
        {
            ...
            for (const Action *A : C.getActions())
                {
                    if (A->getType() != types::TY_Nothing && A->getType() != types::TY_DX_CONTAINER && !(A->getKind() == Action::IfsMergeJobClass || (A->getType() == clang::driver::types::TY_IFS_CPP && A->getKind() == clang::driver::Action::CompileJobClass && 0 == NumIfsOutputs++) || (A->getKind() == Action::BindArchClass && A->getInputs().size() && A->getInputs().front()->getKind() == Action::IfsMergeJobClass)))
                        ++NumOutputs;
                    else if (A->getKind() == Action::OffloadClass && A->getType() == types::TY_Nothing && !C.getArgs().hasArg(options::OPT_fsyntax_only))
                        NumOutputs += A->size();
                }
    ...
    if (Diags.hasErrorOccurred() || C.getArgs().hasArg(options::OPT_Qunused_arguments))
        return;
...
class ToolSelector final
{
                                            ...
                                            bool CanBeCollapsed = true)
    {
        ...
        if (CanBeCollapsed && !CurAction->isCollapsingWithNextDependentActionLegal())
            return nullptr;
    ...
    {
        return TC.useIntegratedAs() && !SaveTemps && !C.getArgs().hasArg(options::OPT_via_file_asm) && !C.getArgs().hasArg(options::OPT__SLASH_FA) && !C.getArgs().hasArg(options::OPT__SLASH_Fa) && !C.getArgs().hasArg(options::OPT_dxc_Fc);
    }

    /// Return true if a preprocessor action can be collapsed.
    bool canCollapsePreprocessorAction() const
    {
        return !C.getArgs().hasArg(options::OPT_no_integrated_cpp) && !C.getArgs().hasArg(options::OPT_traditional_cpp) && !SaveTemps && !C.getArgs().hasArg(options::OPT_rewrite_objc);
    }
                                      ...
                                      ActionList &CollapsedOffloadAction)
    {
        ...
        for (size_t i = 1; i < ActionInfo.size(); i++)
            if (ActionInfo[i].JA->getType() != types::TY_LLVM_BC && ActionInfo[i].JA->getType() != types::TY_LTO_BC)
                {
...
public:
    ToolSelector(const JobAction *BaseAction, const ToolChain &TC,
        ...
        : TC(TC), C(C), BaseAction(BaseAction), SaveTemps(SaveTemps), EmbedBitcode(EmbedBitcode)
    {
    ...
    if (auto *UA = dyn_cast<OffloadUnbundlingJobAction>(JA))
        {
            ...
            for (auto &UI : UA->getDependentActionsInfo())
                {
                    assert(UI.DependentOffloadKind != Action::OFK_None && "Unbundling with no offloading??");
    ...
    else
        {
            ...
            std::string OffloadingPrefix = Action::GetOffloadingFileNamePrefix(
                ...
                /*CreatePrefixForHost=*/isa<OffloadPackagerJobAction>(A) || !(A->getOffloadingHostActiveKinds() == Action::OFK_None || AtTopLevel));
            Result = InputInfo(A, GetNamedOutputPath(C, *JA, BaseInput, BoundArch, AtTopLevel, MultipleArchs, OffloadingPrefix),
                               BaseInput);
    ...
    if (!llvm::sys::path::has_extension(ArgValue))
        {
            ...
            if (FileType == types::TY_Image && Args.hasArg(options::OPT__SLASH_LD, options::OPT__SLASH_LDd))
                {
...
static bool HasPreprocessOutput(const Action &JA)
{
    ...
    if (isa<OffloadAction>(JA) && isa<PreprocessJobAction>(JA.getInputs()[0]))
        return true;
    if (isa<OffloadBundlingJobAction>(JA) && HasPreprocessOutput(*(JA.getInputs()[0])))
        return true;
    return false;
}
...
{
    assert(isa<PrecompileJobAction>(JA) && JA.getType() == types::TY_ModuleFile && (C.getArgs().hasArg(options::OPT_fmodule_output) || C.getArgs().hasArg(options::OPT_fmodule_output_EQ)));
                                       ...
                                       StringRef OffloadingPrefix) const
{
    ...
    if (JA.getType() == types::TY_ModuleFile && C.getArgs().getLastArg(options::OPT_module_file_info))
        {
            return "-";
        }

    if (JA.getType() == types::TY_PP_Asm && C.getArgs().hasArg(options::OPT_dxc_Fc))
        {
    ...
    if (JA.getType() == types::TY_Object && C.getArgs().hasArg(options::OPT_dxc_Fo))
        {
    ...
    if (JA.getType() == types::TY_PP_Asm && (C.getArgs().hasArg(options::OPT__SLASH_FA) || C.getArgs().hasArg(options::OPT__SLASH_Fa)))
        {
    ...
    if (JA.getType() == types::TY_API_INFO && C.getArgs().hasArg(options::OPT_emit_extension_symbol_graphs) && C.getArgs().hasArg(options::OPT_o))
    ...
    bool SpecifiedModuleOutput =
        C.getArgs().hasArg(options::OPT_fmodule_output) || C.getArgs().hasArg(options::OPT_fmodule_output_EQ);
    ...
    // `-fmodule-output`.
    if (!AtTopLevel && isa<PrecompileJobAction>(JA) && JA.getType() == types::TY_ModuleFile && SpecifiedModuleOutput)
        {
    ...
    if ((!AtTopLevel && !isSaveTempsEnabled() && !C.getArgs().hasArg(options::OPT__SLASH_Fo)) || CCGenDiagnostics)
        {
            ...
            bool NeedUniqueDirectory =
                (JA.getOffloadingDeviceKind() == Action::OFK_None || JA.getOffloadingDeviceKind() == Action::OFK_Host) && Triple.isOSDarwin();
    ...
    if ((JA.getType() == types::TY_Object || JA.getType() == types::TY_LTO_BC) && C.getArgs().hasArg(options::OPT__SLASH_Fo, options::OPT__SLASH_o))
        {
    ...
    else if (JA.getType() == types::TY_Image && C.getArgs().hasArg(options::OPT__SLASH_Fe, options::OPT__SLASH_o))
        {
    ...
    else if (JA.getType() == types::TY_Image)
        {
            if (IsCLMode())
            ...
            else
                {
                    ...
                    // unit.
                    bool IsHIPNoRDC = JA.getOffloadingDeviceKind() == Action::OFK_HIP && !C.getArgs().hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false);
    ...
    else if ((JA.getType() == types::TY_Plist || JA.getType() == types::TY_AST) && C.getArgs().hasArg(options::OPT__SLASH_o))
        {
            StringRef Val =
                C.getArgs()
    ...
    else
        {
            ...
            auto IsAMDRDCInCompilePhase = [](const JobAction &JA,
                ...
                return isa<CompileJobAction>(JA) && ((JA.getOffloadingDeviceKind() == Action::OFK_HIP && Args.hasFlag(options::OPT_fgpu_rdc, options::OPT_fno_gpu_rdc, false)) || (JA.getOffloadingDeviceKind() == Action::OFK_OpenMP && TC && TC->getTriple().isAMDGPU()));
            };
            if (!AtTopLevel && JA.getType() == types::TY_LLVM_BC && (C.getArgs().hasArg(options::OPT_emit_llvm) || IsAMDRDCInCompilePhase(JA, C.getArgs())))
                Suffixed += ".tmp";
            Suffixed += '.';
            Suffixed += Suffix;
    ...
    if (!AtTopLevel && isSaveTempsObj() && C.getArgs().hasArg(options::OPT_o) && JA.getType() != types::TY_PCH)
        {
            Arg *FinalOutput = C.getArgs().getLastArg(options::OPT_o);
                    ...
                    case llvm::Triple::ELFIAMCU:
                        if (Target.getArch() == llvm::Triple::hexagon)
                            TC = std::make_unique<toolchains::HexagonToolChain>(*this, Target,
                                                                                Args);
                        else if ((Target.getVendor() == llvm::Triple::MipsTechnologies) && !Target.hasEnvironment())
...
bool Driver::ShouldUseClangCompiler(const JobAction &JA) const
{
    ...
    if (JA.size() != 1 || !types::isAcceptedByClang((*JA.input_begin())->getType()))
        return false;

    // And say "no" if this is not a kind of action clang understands.
    if (!isa<PreprocessJobAction>(JA) && !isa<PrecompileJobAction>(JA) && !isa<CompileJobAction>(JA) && !isa<BackendJobAction>(JA) && !isa<ExtractAPIJobAction>(JA))
        return false;
...
bool Driver::ShouldUseFlangCompiler(const JobAction &JA) const
{
    ...
    if (JA.size() != 1 || !types::isAcceptedByFlang((*JA.input_begin())->getType()))
        return false;

    // And say "no" if this is not a kind of action flang understands.
    if (!isa<PreprocessJobAction>(JA) && !isa<PrecompileJobAction>(JA) && !isa<CompileJobAction>(JA) && !isa<BackendJobAction>(JA))
        return false;
                                   ...
                                   llvm::StringSet<> &SavedStrings)
{
    ...
    else if (Edit[0] == 's' && Edit[1] == '/' && Edit.ends_with("/") && Edit.slice(2, Edit.size() - 1).contains('/'))
        {
